...
Similar to the previous version, AudioVideoAnaysis 2.0 is intended for generating video/motiongram and spectrograms together in realtime. Spectrograms are visual representations of the frequency spectrum of an audio signal as it varies over time. The frequencies are presented along the Y-axis, ranging from lowest to highest (where highest is half of your current sampling rate). Time is then represented on the X-axis from left to right. A videogram is an image representation of a videos movement over time. therefore, we can think about the videogram as the spectrogram of the video domain. We call these images for spectral representations because they visualize the spectral content, or the various spectrums, of any medium. In audio programming and Sonology, this shift is often referred to as moving our attention from the time domain to the frequency domain.
